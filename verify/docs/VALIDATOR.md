...
```go
import "github.com/he-end/verify-reverse/verify/service"
...
```go
// file: verify/handler/register_simulator/build_validator.go
...
```go
// verify/handler/register_simulator/register.go
...
Edit fungsi `buildDetail()` di `verify/service/validation.go`:
...
| File | Peran |
|------|-------|
| `verify/service/validation.go` | `Validator` struct, `Report`, `FieldError`, converter |
| `verify/handler/register_simulator/build_validator.go` | Registrasi custom rules per handler |
| `verify/handler/register_simulator/register.go` | Pemakaian `h.val.Struct()` di handler |
| `verify/route.go` | Wiring `service.Default()` ke handler constructor |
